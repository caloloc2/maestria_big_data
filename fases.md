...
- _2026-08-27 (pm) · **Reproceso por fechas (bloques a demanda) + umbral Gemini unificado a 600 s** · (1) **Calendario de particiones ampliado a TODO el histórico:** `DailyPartitionsDefinition(start_date="2018-01-01")` sin `end_date` (se extiende hasta hoy). Antes estaba limitado a mayo 2025 → Dagster rechazaba cualquier otra fecha. Ahora es materializable **cualquier día o rango** (validado: 2019/2025/2026 → `has_partition_key=True`). El `bronze_audio_index` es global y ya cubre las fechas; al materializar un día, `bronze_audio` **aterriza el MP3 de ese día de Asterisk a Bronce justo-a-tiempo** (SFTP solo lectura) y la cadena `bronze_cdr→bronze_audio→silver_calls→silver_transcriptions→gold_evaluations` procesa solo ese bloque. Comando por día: `docker exec -e ASR_LIMIT=0 uisrael_dagster_webserver bash -lc "cd /opt/dagster/app && dagster asset materialize -f src/definitions.py --select bronze_cdr,bronze_audio,silver_calls,silver_transcriptions,gold_evaluations --partition YYYY-MM-DD"`; rango = backfill de la UI o bucle. Esto reemplaza el backfill masivo de 424 GB: Bronce se llena incrementalmente con lo que se reprocesa (decisión del tesista, mejor que pre-cargar todo). (2) **Umbral de envío a Gemini subido de 300 s a 600 s (10 min), en streaming Y batch:** streaming `STREAM_EVAL_MIN_SECS=600` (runner + compose, recreado); batch **nuevo filtro** en `gold_evaluations` (`EVAL_MIN_SECS=600`): solo evalúa en Gemini las grabaciones >600 s; las cortas quedan transcritas/anonimizadas pero NO se mandan al LLM (ahorro de tokens, mismo criterio que streaming). **Validado sin gastar tokens de más:** `gold_evaluations` de 2025-05-14 (25 transcripciones, 1 larga) evaluó **solo la de 1481 s** y omitió 24 cortas. `ASR_LIMIT` sigue en 60 por defecto (poner `ASR_LIMIT=0` para el día completo)._
- _2026-08-27 (pm) · **FIX: diarización APAGADA por defecto en batch (antes diarizaba en línea y era el cuello) + vista "Detalle de llamada" en el tablero** · **Hallazgo (validado en vivo):** al reprocesar 2026-08-03, el batch estaba **diarizando en línea en CPU** (pyannote, RTF ~0,6–0,8) porque `silver_transcriptions` encolaba los `asr.jobs` SIN el flag `diarize` y el worker usaba su default `DIARIZE=1`. Efecto medible: el reproceso tardó ~32 min y `silver_transcriptions` **perdió 14 de 28** llamadas por el timeout de 30 min (`ASR_TIMEOUT=1800`). Inconsistente con la decisión de "diarización diferida/apagada por defecto" (que sí estaba en streaming). **Arreglo:** (1) `whisper_worker/worker.py` default `DIARIZE=0` (solo diariza si el JOB lo pide explícito → únicamente `gold_diarizations`); (2) `silver_transcriptions` ahora manda `diarize=false` y **marca las largas** (`dur≥DIARIZE_MIN_SECS`, def. 600) con `requiere_diarizacion=true`, igual que el streaming, para diferirlas a `gold_diarizations`. **Validado (reproceso 2026-08-04):** todas `hablantes=None` (sin diarizar) y **RTF rápido en GPU** — llamada de 1472 s → proc 122 s (RTF ~0,08), 1626 s → 96 s (~0,06), cortas ~0,02–0,04; sin timeouts. **Además — punto 2 del tesista:** nueva **7.ª pestaña "🔎 Detalle de llamada"** en `src/dashboard/app.py`: lista TODAS las transcripciones del filtro (críticas y alto riesgo primero), y al elegir una muestra la **transcripción anonimizada completa** + si tiene evaluación de Gemini, el desglose (calidad, es_venta/venta_válida, riesgo, infracciones grupo B, omisiones C, ítems A, sentimiento asesor/cliente, modelo/confianza); si no tiene evaluación (≤10 min), solo la transcripción. Requiere reiniciar el contenedor `dashboard` para recargar. **Nota operativa del tesista (a documentar, sin automatizar aún):** el batch se correrá **bajo demanda en horas valle (madrugada)** y el streaming en horario laboral → ver [[tesis-modelo-operativo-batch-streaming]]._
